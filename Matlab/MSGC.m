function result = MSGC(X,grid_num)
[n, ~] = size(X);

%% Generate the grid using the minimum and maximum of coordinates
x_gap = (max(X(:,1))-min(X(:,1)))/grid_num;
y_gap = (max(X(:,2))-min(X(:,2)))/grid_num;
...
for i=1:n
    x_index = ceil((X(i,1)-min(X(:,1)))/x_gap);
    y_index = ceil((X(i,2)-min(X(:,2)))/y_gap);  
    if(x_index==0)
        x_index=1;
    end
    if(y_index==0)
